...
def get_args():
    ...
    parser.add_argument('-o', '--output',
                        help='output image', default='lambda_ablation.png')
...
def do_plot(data_xs, data_ys):
    ...
    """
    fig = plt.figure(figsize=(16.18 / 1.5, 10 / 1.5))
...
if __name__ == '__main__':
    ...
    old_x, old_y = np.array([1e-2, 1e-1,  1e1, 1e2]), np.array([0.07, 42.58,  67.59, 17.68])
    new_x, new_y = np.array([1e-2, 1e-1,  1e1, 1e2]), np.array([64.49, 64.48,  63.27, 7.2])
    all_x, all_y = np.array([1e-2, 1e-1,  1e1, 1e2]), np.array([32.28, 53.53, 65.48, 12.44])

    get_args()
    args.title = ""#Relation between shot number and validation mIoU"
    args.ylabel = "validation mIoU"
    args.xlabel = "lambda"
    args.legend = "old,new,all"
    args.y_column = "y,y,y,y"
    args.x_column = "x"
    #args.ylim = [65,69]

    do_plot(data_xs = [new_x], data_ys = [old_y, new_y, all_y])