...
POWER = 0.9
RANDOM_SEED = 1234
RESTORE_FROM = 'train_log/train.473.class10meaning.filtered.old.epoch_eval.densenet201/love.pth' #'http://download.pytorch.org/models/resnet50-19c8e357.pth'
...
def main():
    ...
    # Create network.
    teacher_model = densenet201(num_classes=teacher_class_num,pre_softmax_node=1920)

    student_model = densenet201(num_classes=student_class_num,pre_softmax_node=1920)
    ...
    student_remove_classifier = {}
    for i in saved_state_dict:
        i_parts = i.split(".")
        if i_parts[0] == 'classifier':
            continue
        student_remove_classifier[i] = saved_state_dict[i]
        logger.info("recovering weight for  model(loading resnet weight): {}".format(i))

    student_model.load_state_dict(student_remove_classifier, strict=False)
    teacher_model.load_state_dict(saved_state_dict, strict=False)
    ...
    for param in student_model.features.conv0.parameters():
        param.requires_grad = False
    for param in student_model.features.norm0.parameters():
        param.requires_grad = False


    for param in student_model.features.denseblock1.parameters():
        param.requires_grad = False

    for param in student_model.features.denseblock2.parameters():
        param.requires_grad = False
...
if __name__ == '__main__':
    if args.test:
        ...
        student_model = densenet201(num_classes=student_class_num)
