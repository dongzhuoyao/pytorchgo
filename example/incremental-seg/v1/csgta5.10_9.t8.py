...
def main():
    ...
    for i_iter, batch in tqdm(enumerate(trainloader), total=len(trainloader), desc="training deeplab"):
        ...
        if i_iter % 50 == 0:
            logger.info('loss = {}, seg_loss = {}, distill_loss = {}, lr = {}, best_miou_new = {}'.format(
                loss.data.cpu().numpy(),
        ...
        if i_iter % args.save_pred_every == 0 and i_iter != 0:
            ...
            if is_best:
                ...
                logger.info("val miou = {}".format(np.mean(best_val_ious)))
    ...
    student_model.eval()
    test_ious = do_eval_offline(model=student_model, is_save=False,
                                data_dir=args.data_dir, data_list=TEST_DATA_LIST_PATH,
                                num_classes=student_class_num, handinhand=False)
    ...
    logger.info("test miou = {}".format(np.mean(test_ious)))
...
if __name__ == '__main__':
    if args.test:
        ...
        args.test_restore_from = "train_log/csgta5.10_8.t1/love.pth"
        from evaluate_incremental_csgta5 import do_eval_offline

        test_ious = do_eval_offline(model=student_model, restore_from=args.test_restore_from, is_save=False,
                                    data_dir=args.data_dir, data_list=TEST_DATA_LIST_PATH,
                                    num_classes=student_class_num, handinhand=False)
