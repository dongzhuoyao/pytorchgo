...
BATCH_SIZE = 8
DATA_DIRECTORY = '/home/hutao/dataset/incremental_seg/coco2voc'
DATA_LIST_PATH = 'datalist/coco2voc_split_class10+10/old/coco2voc_split_train.txt'
VAL_DATA_LIST_PATH = 'datalist/coco2voc_split_class10+10/old/coco2voc_split_val.txt'
NUM_CLASSES = 10+1
...
def main():
    ...
    trainloader = data.DataLoader(VOCDataSet(args.data_dir, args.data_list, max_iters=args.num_steps*args.batch_size,
                     mirror=args.random_mirror, img_transform=img_transform, label_transform=label_transform, augmentation=augmentation, img_prefix="image"),
    ...
    for i_iter, batch in tqdm(enumerate(trainloader), total=len(trainloader), desc="training deeplab"):
        ...
        if i_iter >= args.num_steps-1:
            ...
            from evaluate_incremental import do_eval_coco2voc
            model.eval()
            ious = do_eval_coco2voc(model=model, data_dir=args.data_dir, data_list=VAL_DATA_LIST_PATH, num_classes=NUM_CLASSES, quick_eval=quick_eval)
        ...
        if i_iter % args.save_pred_every == 0 and i_iter!=0:
            ...
            from evaluate_incremental import do_eval_coco2voc
            model.eval()
            ious = do_eval_coco2voc\
                (model=model, data_dir=args.data_dir, data_list=VAL_DATA_LIST_PATH, num_classes=NUM_CLASSES, quick_eval=quick_eval)
            cur_miou = np.mean(ious[1:])
            model.train()
...
if __name__ == '__main__':
    if args.test:
        ...
        from evaluate import do_eval_coco2voc
        ...
        student_model.eval()
        do_eval_coco2voc(model=student_model, restore_from=args.test_restore_from, data_dir=args.data_dir, data_list=VAL_DATA_LIST_PATH, num_classes=NUM_CLASSES)
    else:
        logger.auto_set_dir()
        main()
