...
def save_flows(flows,image,save_dir,num,bound):
    ...
    #save the image
    save_img=os.path.join(data_root,new_dir,save_dir,'{:05d}.jpg'.format(num))
    scipy.misc.imsave(save_img,image)

    #save the flows
    save_x=os.path.join(data_root,new_dir,save_dir,'{:05d}_flow_x.jpg'.format(num))
    save_y=os.path.join(data_root,new_dir,save_dir,'{:05d}_flow_y.jpg'.format(num))
