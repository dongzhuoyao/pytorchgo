...
class MyTrainer_ROAD(object):
    ...
    def train_epoch(self):
        ...
        self.G_source_loader_iter = enumerate(self.train_loader)#[enumerate(self.train_loader) for _ in range(G_STEP)]
        self.G_target_loader_iter = enumerate(self.target_loader)#[enumerate(self.target_loader)  for _ in range(G_STEP)]

        self.D_source_loader_iter = enumerate(self.train_loader)#[enumerate(self.train_loader) for _ in range(D_STEP)]
        self.D_target_loader_iter = enumerate(self.target_loader)#[enumerate(self.target_loader)  for _ in range(D_STEP)]
        ...
        #for _ in range(D_STEP):
            ...
            src_discriminate_result = self.netD(F.softmax(score), is_reverse=True)
            target_discriminate_result = self.netD(F.softmax(seg_target_score), is_reverse=True)
        ...
        #for _ in range(G_STEP):
            source_data, source_labels, target_data = get_data(self.G_source_loader_iter, self.G_target_loader_iter)
