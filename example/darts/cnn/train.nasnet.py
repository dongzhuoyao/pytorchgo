...
args.save = 'NASNet-{}-{}'.format(args.save, time.strftime("%Y%m%d-%H%M%S"))
...
def main():
  ...
  model = Network(C=args.init_channels, num_classes=CIFAR_CLASSES, layers=args.layers, auxiliary=args.auxiliary, genotype=genotype)
  model = model.cuda()
...
def train(train_queue, model, criterion, optimizer, epoch_str):
  ...
  for step, (input, target) in tqdm(enumerate(train_queue),total=len(train_queue), desc="training {}".format(epoch_str)):
...
def infer(valid_queue, model, criterion):
  ...
  for step, (input, target) in tqdm(enumerate(valid_queue),total=len(valid_queue),desc="validation"):
