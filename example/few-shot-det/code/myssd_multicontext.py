...
class SSD(nn.Module):
    ...
    def __init__(self, size, base, extras, head, num_classes, start_channels=3, top_k=200, conf_thresh=0.01, nms_thresh=0.45, fuse_conv5=False, fuse_conv45=False, fuse_conv345=False,fuse_conv4=False):
        ...
        from vgg_fcn_multicontext import vgg16
    ...
    def forward(self, support_images, x, is_train):
        ...
        support_result, feature_list = self.support_net(support_images)
        ...
        if self.fuse_conv5:
            ...
            for k in range(23, len(self.vgg)):
                if k == 29:
                    x = self.vgg[k](x) + feature_list[-1]#support !!!
                elif k == 27:
                    x = self.vgg[k](x) + feature_list[-2] # support !!!
                elif k == 25:
                    x = self.vgg[k](x) + feature_list[-3] # support !!!
                else:
                    x = self.vgg[k](x)
            sources.append(x)
        elif self.fuse_conv45:
            # apply vgg up to conv4_3 relu
            for k in range(23):
                if k == 22:
            ...
            for k in range(23):
                if k == 22:
                    ...
                    x = self.vgg[k](x) + feature_list[-7]  # support !!!
                elif k == 13:
                    x = self.vgg[k](x) + feature_list[-8]  # support !!!
                elif k == 11:
                    x = self.vgg[k](x) + feature_list[-9]  # support !!!
...
def multibox(vgg, extra_layers, cfg, num_classes):
    ...
    for k, v in enumerate(vgg_source):
        try:
            ...
            import ipdb
            ipdb.set_trace()
    for k, v in enumerate(extra_layers[1::2], start=2):#k start from 2
