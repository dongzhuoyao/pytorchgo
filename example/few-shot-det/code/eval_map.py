...
parser.add_argument('-q', '--quiet', help="minimalistic console output.", default=True, action="store_true")
...
def eval_online(basedir, no_animation=True, no_plot=True,quiet= True, ignore=None,set_class_iou=None):
      ...
      predicted = os.path.join(basedir,'predicted')
      ground_truth = os.path.join(basedir,'ground-truth')
      tmp_files_path = os.path.join(basedir,"tmp_files")
      ...
      ground_truth_files_list = glob.glob(os.path.join(ground_truth,'*.txt'))
      ...
      for txt_file in ground_truth_files_list:
        ...
        if not os.path.exists(os.path.join(predicted, file_id + ".txt")):
      ...
      predicted_files_list = glob.glob(os.path.join(predicted,'*.txt'))
      ...
      for class_index, class_name in enumerate(gt_classes):
        ...
        for txt_file in predicted_files_list:
          ...
          file_id = os.path.basename(os.path.normpath(file_id))
          if class_index == 0:
            if not os.path.exists(os.path.join(ground_truth,file_id + ".txt")):
