...
def train(args):
    ...
    if hasattr(model, 'optimizer'):
        ...
        optimizer = torch.optim.SGD(model.optimizer_params(args.l_rate), lr=args.l_rate, momentum=0.99,
                                    weight_decay=5e-4)
    ...
    for epoch in tqdm(range(args.n_epoch), total=args.n_epoch):
        ...
        for i, (images, labels) in tqdm(enumerate(trainloader), total=len(trainloader),
            ...
            if (i + 1) % 100 == 0:
                logger.info("Epoch [%d/%d] Loss: %.4f, lr: %.7f, best mIoU: %.7f" % (
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', nargs='?', type=int, default=3,
                        help='Batch Size')
    parser.add_argument('--l_rate', nargs='?', type=float, default=2.5e-4,
                        # original implementation of deeplabv1 learning rate is 1e-3 and poly update
                        help='Learning Rate')
